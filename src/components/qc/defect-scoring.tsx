...
const SeverityOnlyDefectField = ({ control, categoryId, defectKey, label }: { control: any, categoryId: string, defectKey: string, label: string }) => {
                ...
                <div className="flex flex-wrap gap-2 justify-end max-w-md">
                 ...
                 <Dialog>
                    ...
                    <DialogContent className="sm:max-w-md">
                        <DialogHeader>
                            <DialogTitle>Add Severity Score for {label}</DialogTitle>
                        </DialogHeader>
                        <div className="py-4">
                            <Label>Severity</Label>
                            <Select onValueChange={(val) => handleAddSeverity(Number(val))}>
            ...
            {category.defects.map((defect) => (
              <div key={defect.key}>
                {category.id === 'lamination' ? (
                    ...
                    <SeverityOnlyDefectField control={control} categoryId={category.id} defectKey={defect.key} label={defect.label} />
                )}
              </div>
            ))}
          </AccordionContent>
